...
namespace pf {
		...
		for (int x = 0; x < simulation_mesh::phase_field.Nx(); x++)
			for (int y = 0; y < simulation_mesh::phase_field.Ny(); y++)
				for (int z = 0; z < simulation_mesh::phase_field.Nz(); z++) {
					simulation_mesh::phase_field(x, y, z).clear();
				}
		simulation_mesh::phase_field.clear();
	}
	inline void init_phase_field_equations() {
		...
		if (InputFileReader::get_instance()->read_int_value("Solver.ControlEquation.PhaseField", control_equation, true)) {
			// - 
			model_parameters::is_phi_field_on = true;
			// - 
			model_parameters::phi_equation = PhaseFieldEquation(control_equation);
			// - 
			model_parameters::phi_property_number = int(materials_system::PHASES.size());
			InputFileReader::get_instance()->read_int_value("Solver.ControlEquation.PhaseField.PhaseNumber", model_parameters::phi_number, true);
			model_parameters::phi_property.resize(model_parameters::phi_number);
			simulation_mesh::phase_field.init(mesh_parameters::MESH_NX, mesh_parameters::MESH_NY, mesh_parameters::MESH_NZ);
			for (int x = 0; x < simulation_mesh::phase_field.Nx(); x++)
				for (int y = 0; y < simulation_mesh::phase_field.Ny(); y++)
					for (int z = 0; z < simulation_mesh::phase_field.Nz(); z++) {
						simulation_mesh::phase_field(x, y, z).init(model_parameters::phi_number);
					}
			// - 
			...
			for (int index = 0; index < model_parameters::phi_number; index++)
				model_parameters::phi_property[index] = -1;
			if (model_parameters::phi_property_number != 0)
			...
			for (int index = 0; index < model_parameters::phi_property_number; index++) {
				...
				for (int index2 = 0; index2 < phi_property_value.size(); index2++)
					if (model_parameters::phi_property[phi_property_value[index2].int_value] == -1)
						model_parameters::phi_property[phi_property_value[index2].int_value] = index;
					else {
			...
			for (int index = 0; index < model_parameters::phi_number; index++)
				if (model_parameters::phi_property[index] == -1)
			...
			switch (model_parameters::phi_equation)
			{
		...
		for (int k = write_vts::z_begin; k <= write_vts::z_end; ++k)
			for (int j = write_vts::y_begin; j <= write_vts::y_end; ++j)
				for (int i = write_vts::x_begin; i <= write_vts::x_end; ++i) {
					PhaseFieldPoint& point = simulation_mesh::phase_field(i, j, k);
					REAL fix = 0.0;
					for (int index = 0; index < model_parameters::phi_number; index++)
		...
		for (int k = write_vts::z_begin; k <= write_vts::z_end; ++k)
			for (int j = write_vts::y_begin; j <= write_vts::y_end; ++j)
				for (int i = write_vts::x_begin; i <= write_vts::x_end; ++i) {
					...
					REAL fix = 0.0;
					for (int index = 0; index < model_parameters::phi_number; index++)
						fix += point.phi[index] * model_parameters::phi_property[index];
					fout << fix << endl;
				}
		fout << "</DataArray>" << endl;
	}
			...
			for (int k = write_vts::z_begin; k <= write_vts::z_end; ++k)
				for (int j = write_vts::y_begin; j <= write_vts::y_end; ++j)
					for (int i = write_vts::x_begin; i <= write_vts::x_end; ++i) {
						...
						REAL fix = 0.0;
						for (int index = 0; index < model_parameters::phi_number; index++)
							if (pindex == model_parameters::phi_property[index])
								fix += point.phi[index];
						fout << fix << endl;
					}
			fout << "</DataArray>" << endl;
	...
	inline void write_scalar_phi_all(ofstream& fout) {
		for (int pindex = 0; pindex < model_parameters::phi_number; pindex++) {
			...
			for (int k = write_vts::z_begin; k <= write_vts::z_end; ++k)
				for (int j = write_vts::y_begin; j <= write_vts::y_end; ++j)
					for (int i = write_vts::x_begin; i <= write_vts::x_end; ++i)
						fout << simulation_mesh::phase_field(i, j, k).phi[pindex] << endl;
			fout << "</DataArray>" << endl;
		}
	}
}