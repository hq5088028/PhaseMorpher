...
namespace pf {
	namespace automatic_change_delt_time {
		...
		void init_auto_time() {
			InputFileReader::get_instance()->read_int_value("Postprocess.PCT.TimeInterval.frequence", delt_step, true);
			if (delt_step > 0) {
				time_interval = time_parameters::delt_t;
				InputFileReader::get_instance()->read_REAL_value("Postprocess.PCT.TimeInterval.large_scale", max_scale, true);
				InputFileReader::get_instance()->read_bool_value("Postprocess.PCT.TimeInterval.is_reduce_output", is_reduce_output, true);
				InputFileReader::get_instance()->read_REAL_value("Postprocess.PCT.TimeInterval.phi_increment_limit", phi_increment_limit, true);
				InputFileReader::get_instance()->read_REAL_value("Postprocess.PCT.TimeInterval.con_increment_limit", con_increment_limit, true);
				InputFileReader::get_instance()->read_REAL_value("Postprocess.PCT.TimeInterval.temp_increment_limit", temp_increment_limit, true);
