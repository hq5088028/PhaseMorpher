...
namespace pf {
	...
	public:
		...
		REAL getGrayPercentage(int x, int y) {
			vector<int> rgb;
			rgb.push_back(bmpbuf[y * linebyte + x * 3 + 2]);
			rgb.push_back(bmpbuf[y * linebyte + x * 3 + 1]);
			rgb.push_back(bmpbuf[y * linebyte + x * 3 + 0]);
			return (rgb[0] * REAL(0.299) + rgb[1] * REAL(0.587) + rgb[2] * REAL(0.114)) / 255;
		}
		...
		void GrayTransfer() {
			for(int x = 0; x < bmp_width; x++)
				for (int y = 0; y < bmp_height; y++) {
					...
					int gray = REAL_to_int(rgb[0] * REAL(0.299) + rgb[1] * REAL(0.587) + rgb[2] * REAL(0.114));
